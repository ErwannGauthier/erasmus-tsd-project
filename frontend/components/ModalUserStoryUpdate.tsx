...
interface ModalProps {
  roomId: string,
  formDataProps: FormDataUpdate;
  closeModal: () => void;
}

const ModalUserStoryUpdate: React.FC<ModalProps> = ({ roomId, formDataProps, closeModal }) => {
  const [formData, setFormData] = useState<FormDataUpdate>(formDataProps);
  ...
  const handleUpdate = async (event: React.FormEvent): Promise<void> => {
    ...
    socket.emit('updateUserStory', { roomId: roomId, ...formData }, (response: CommonResponse) => {
      if (!response.isOk) {
  ...
  const handleDelete = async (event: React.FormEvent): Promise<void> => {
    ...
    socket.emit('deleteUserStory', { roomId: roomId, userStoryId: formData.userStoryId });
    closeModal();
  };
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
      <div className="bg-white p-6 rounded shadow-lg w-96">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl">Update User Story</h2>
        ...
        <div>
          ...
          <form onSubmit={handleUpdate} id="formUpdate"
            ...
            <div className="pb-4">
              ...
              <input
                ...
                value={formData.name}
                onChange={(event: React.ChangeEvent<HTMLInputElement>) => setFormData((previousForm: FormDataUpdate) => ({
                  ...previousForm,
            ...
            <div className="pb-4">
              ...
              <textarea
                ...
                onChange={(event: React.ChangeEvent<HTMLTextAreaElement>) => setFormData((previousForm: FormDataUpdate) => ({
                  ...previousForm,
                  [event.target.name]: event.target.value
                }))}
              ></textarea>
          ...
          <form onSubmit={handleDelete} id="formDelete"
                className="flex flex-col text-black">
          </form>
          <div className="flex justify-end gap-2">
            <button id="submitUpdateButton" type="submit" form="formUpdate"
                    className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">
              Update
            </button>
            <button id="submitDeleteButton" type="submit" form="formDelete"
...
export default ModalUserStoryUpdate;