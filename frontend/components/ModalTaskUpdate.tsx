...
const ModalTaskUpdate: React.FC<ModalProps> = ({ roomId, formDataProps, closeModal }) => {
  const [formData, setFormData] = useState<Task>(formDataProps);
  ...
  const handleUpdate = async (event: React.FormEvent): Promise<void> => {
    ...
    socket.emit('updateTask', { roomId: roomId, ...formData }, (response: CommonResponse) => {
      if (!response.isOk) {
  ...
  const handleDelete = async (event: React.FormEvent): Promise<void> => {
    ...
    socket.emit('deleteTask', { roomId: roomId, taskId: formData.taskId });
    closeModal();
  };
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
      <div className="bg-white p-6 rounded shadow-lg w-96">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl">Update Task</h2>
          ...
          <form onSubmit={handleUpdate} id="formUpdate"
            ...
            <div className="pb-4">
              ...
              <input
                ...
                value={formData.name}
                onChange={(event: React.ChangeEvent<HTMLInputElement>) => setFormData((previousForm: Task) => ({
                  ...previousForm,
            ...
            <div className="pb-4">
              ...
              <textarea
                ...
                onChange={(event: React.ChangeEvent<HTMLTextAreaElement>) => setFormData((previousForm: Task) => ({
                  ...previousForm,
                  [event.target.name]: event.target.value
                }))}
              ></textarea>
